...
public class HomeController {
  ...
  public String conImporte(@RequestParam("idventa") int idventa, Model model) {
    try {
      // Proceso
  ...
  public String conPublicaciones(@ModelAttribute Publicacion bean, Model model) {
  ...
  @RequestMapping(value="/regVenta.htm", method=RequestMethod.GET )
  public String regVenta(){
    return "regVenta";
  }

  
  
  @RequestMapping(value="/regVenta.htm", method=RequestMethod.POST )
  public String regVenta( @ModelAttribute Venta venta, Model model){
    
    try {
      venta.setIdempleado(3);
      ventaService.regVenta(venta);
